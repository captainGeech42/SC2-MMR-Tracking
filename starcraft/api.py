...
class API:
    ...
    @staticmethod
    def get_all_ladders(max_league_id, season_id, params: dict) -> """List of ladders""":
        ladders = []
        for league_id in range(max_league_id + 1):
            try:
                r = requests.get("https://us.api.battle.net/data/sc2/league/{}/201/0/{}".format(season_id, league_id),
                                 params)
                if r.status_code != 200:
                ...
                for tier in json["tier"]:
                    ...
                    for ladder in tier["division"]:
                        ...
                        obj = Ladder(ladder_id, league_id, division, min_mmr, max_mmr)
                        ladders.append(obj)
    ...
    @staticmethod
    def get_players_in_ladder(ladder: Ladder, params: dict) -> """List of players""":
        players = []
        r = requests.get("https://us.api.battle.net/data/sc2/ladder/{}".format(ladder.id), params)
        if r.status_code != 200:
            raise RequestError(r, "{} returned error code {}, skipping...".format(r.url, r.status_code))
        json = r.json()
